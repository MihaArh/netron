...
view.View = class {
  ...
  async error(err, name, screen) {
    ...
    const knowns = [
      ...
      {
        name: "",
        message: /^Failed to render tensor/,
        url: "https://github.com/lutzroeder/netron/issues/681",
      },
      {
        name: "Error",
  ...
  async open(context) {
    ...
    try {
      ...
      const graphs =
        Array.isArray(model.graphs) && model.graphs.length > 0
          ? [model.graphs[0]]
          : [];
  ...
  get activeGraph() {
    return Array.isArray(this._graphs) && this._graphs.length > 0
      ? this._graphs[0]
      : null;
  }
    ...
    const rotate = graph.nodes.every(
      (node) =>
        node.inputs.filter((input) =>
          input.value.every((argument) => !argument.initializer)
  ...
  export(file) {
    ...
    const extension =
      lastIndex != -1 ? file.substring(lastIndex + 1).toLowerCase() : "png";
    if (this.activeGraph && (extension === "png" || extension === "svg")) {
      ...
      origin.setAttribute(
        "transform",
        "translate(" +
          (delta - size.x).toString() +
          ", " +
          (delta - size.y).toString() +
          ") scale(1)"
      );
  ...
  showModelProperties() {
    if (this._model) {
      try {
        const modelSidebar = new view.ModelSidebar(
          this._host,
          this._model,
          this.activeGraph
        );
        ...
        this._sidebar.open(content, "Model");
      } catch (error) {
        if (error) {
  ...
  showNodeProperties(node) {
    if (node) {
      try {
        ...
        nodeSidebar.on("export-tensor", (sender, tensor) => {
          ...
          this._host.save("NumPy Array", "npy", defaultPath, (file) => {
            try {
              ...
              const array = execution.invoke("numpy.asarray", [
                tensor.value,
                dtype,
              ]);
        ...
        this._sidebar.open(nodeSidebar.render(), "Node");
      } catch (error) {
        if (error) {
          error.context = this._model.identifier;
        }
  ...
  showConnectionProperties(value, from, to) {
    try {
      ...
      const connectionSidebar = new view.ConnectionSidebar(
        this._host,
        value,
        from,
        to
  ...
  showDocumentation(type) {
    if (
      ...
      const documentationSidebar = new view.DocumentationSidebar(
        this._host,
        type
      );
  ...
  _add(node) {
    ...
    if (typeof type.name !== "string" || !type.name.split) {
      ...
      const error = new view.Error(
        "Unsupported node type '" + JSON.stringify(type.name) + "'."
      );
    ...
    if (this.context.view.options.weights) {
      for (const input of node.inputs) {
        if (
          ...
          input.value[0].initializer != null
        ) {
          initializers.push(input);
        }
        if (
          (input.visible === false || input.value.length > 1) &&
    ...
    if (
      ...
      for (const initializer of initializers) {
        ...
        if (
          ...
          if (type.shape.dimensions.length === 0 && value.initializer) {
            try {
              ...
              const error = new view.Error(
                "Failed to render tensor of type '" +
                  type +
                  "' (" +
                  err.message +
                  ")."
        ...
        list.add(
          value.name ? "initializer-" + value.name : "",
          initializer.name,
          shape,
          type ? type.toString() : "",
          separator
        ...
        if (
          ...
          type.shape.dimensions.every(
            (dim) =>
              ...
              dim instanceof base.Int64 ||
              typeof dim === "string"
          )
        ) {
          content = type.shape.dimensions
            .map((dim) => (dim !== null && dim !== undefined ? dim : "?"))
            .join("\u00D7");
...
view.Edge = class extends grapher.Edge {
  ...
  get minlen() {
    if (
      this.from.inputs.every((argument) =>
...
view.NodeSidebar = class extends view.ObjectSidebar {
  ...
  _addAttribute(name, attribute) {
    ...
    switch (attribute.type) {
      case "tensor": {
        value = new view.ValueView(
          this._host,
          { type: attribute.value.type, initializer: attribute.value },
          ""
        );
        value.on("export-tensor", (sender, value) =>
          this.emit("export-tensor", value)
        );
...
view.AttributeView = class extends view.Control {
  ...
  toggle() {
    if (this._expander.innerText == "+") {
      ...
      const typeLine = this.createElement(
        "div",
      ...
      if (description) {
        const descriptionLine = this.createElement(
          "div",
          "sidebar-item-value-line-border"
        );
...
view.ArgumentView = class extends view.Control {
  constructor(host, argument) {
    ...
    for (const value of argument.value) {
      ...
      item.on("export-tensor", (sender, value) =>
        this.emit("export-tensor", value)
      );
...
view.ValueView = class extends view.Control {
  constructor(host, value, name) {
    ...
    if (this._hasId || (!this._hasCategory && !type && !tensor)) {
      ...
      nameLine.innerHTML =
        "<span class='sidebar-item-value-line-content'>name: <b>" +
        (name || " ") +
        "</b></span>";
      nameLine.addEventListener("pointerenter", () =>
        this.emit("activate", this._value)
      );
      nameLine.addEventListener("pointerleave", () =>
        this.emit("deactivate", this._value)
      );
      if (!initializer) {
      ...
      this._code(
        "tensor",
        type.toString().split("<").join("&lt;").split(">").join("&gt;")
      );
    }
  }
  ...
  toggle() {
    if (this._expander) {
      if (this._expander.innerText == "+") {
        ...
        const quantization = this._value.quantization;
        if (quantization) {
          const quantizationLine = this.createElement(
            "div",
            "sidebar-item-value-line-border"
          );
          const content = !Array.isArray(quantization)
            ? quantization
            : "<br><br>" +
              quantization.map((value) => "  " + value).join("<br>");
          quantizationLine.innerHTML =
            "<span class='sidebar-item-value-line-content'>quantization: " +
            "<b>" +
            content +
            "</b></span>";
  ...
  _add(child) {
    child.className =
      this._element.childNodes.length < 2
  ...
  _tensor(value) {
    ...
    try {
      ...
      if (
        ...
        contentLine.innerHTML =
          "Tensor encoding '" + tensor.layout + "' is not implemented.";
      } else if (
        tensor.layout &&
        ...
        contentLine.innerHTML =
          "Tensor layout '" + tensor.layout + "' is not implemented.";
    ...
    const valueLine = this.createElement(
      "div",
      "sidebar-item-value-line-border"
    );
...
view.NodeView = class extends view.Control {
  constructor(host, node) {
    ...
    if (type) {
      ...
      element.innerHTML =
        "<span class='sidebar-item-value-line-content'>node: <b>" +
        (type || " ") +
        "</b></span>";
      ...
      element.addEventListener("pointerleave", () =>
        this.emit("deactivate", this._node)
      );
    ...
    if (!this._elements) {
      ...
      if (Array.isArray(type.outputs) && type.outputs.length > 0) {
        this._append(
          ...
          "h2",
          "Outputs" +
            (type.outputs_range ? " (" + type.outputs_range + ")" : "")
        );
...
view.FindSidebar = class extends view.Control {
  ...
  update(searchText) {
    ...
    this._contentElement.style.display =
      this._contentElement.childNodes.length != 0 ? "block" : "none";
  }
...
view.Tensor = class {
    ...
    switch (this._encoding) {
      ...
      default: {
        throw new view.Error(
          "Unsupported tensor encoding '" + this._encoding + "'."
        );
      }
    }
    switch (this._layout) {
  ...
  get empty() {
    switch (this._layout) {
      ...
      case "sparse.coo": {
        return (
          !this._values || this.indices || this._values.values.length === 0
        );
      }
      default: {
        switch (this._encoding) {
          ...
          case "|":
            return (
              !(
                Array.isArray(this._values) || ArrayBuffer.isView(this._values)
  ...
  get value() {
    ...
    switch (context.encoding) {
      ...
      default: {
        throw new Error(
          "Unsupported tensor encoding '" + context.encoding + "'."
        );
      }
    }
  }
  ...
  _context() {
    if (
      ...
      throw new Error(
        "Tensor encoding '" + this._encoding + "' is not supported."
      );
    }
    if (
      this._layout &&
    ...
    context.dimensions = this._type.shape.dimensions.map((value) =>
      !Number.isInteger(value) && value.toNumber ? value.toNumber() : value
    );
    ...
    switch (this._layout) {
      case "sparse": {
        ...
        context.data = this._decodeSparse(
          dataType,
          context.dimensions,
          indices,
          values
      ...
      case "sparse.coo": {
        ...
        let stride = 1;
        const strides = context.dimensions
          .slice()
          .reverse()
          .map((dim) => {
            const value = stride;
      ...
      default: {
        switch (this._encoding) {
          ...
          case ">": {
            context.data =
              ...
              this._data instanceof Int8Array
                ? this._data
                : this._data.peek();
            context.view = new DataView(
              context.data.buffer,
              context.data.byteOffset,
              context.data.byteLength
            );
            if (view.Tensor.dataTypes.has(dataType)) {
              ...
              !isNaN(parseInt(dataType.substring(4), 10))
            ) {
              ...
              !isNaN(parseInt(dataType.substring(3), 10))
            ) {
              ...
              throw new Error(
                "Tensor data type '" + dataType + "' is not implemented."
              );
            }
            break;
          }
          case "|": {
            ...
            if (
              !view.Tensor.dataTypes.has(dataType) &&
              dataType !== "string" &&
              dataType !== "object"
            ) {
              throw new Error(
  ...
  _decodeData(context, dimension) {
    const results = [];
    const dimensions =
      context.dimensions.length == 0 ? [1] : context.dimensions;
  ...
  static _stringify(value, indentation, indent) {
    if (Array.isArray(value)) {
      ...
      const items = value.map((item) =>
        view.Tensor._stringify(item, indentation + indent, indent)
      );
      if (items.length > 0) {
...
view.Formatter = class {
  ...
  _format(value, type, quote) {
    ...
    if (
      value &&
      (value instanceof base.Int64 || value instanceof base.Uint64)
    ) {
    ...
    if (Array.isArray(value)) {
      ...
      const array = value.map((item) => {
        if (
          item &&
          (item instanceof base.Int64 || item instanceof base.Uint64)
        ) {
      ...
      switch (token.type) {
        ...
        case "code": {
          ...
          html +=
            "<pre><code" +
            (language
              ? ' class="' + "language-" + this._encode(language) + '"'
              : "") +
            ">" +
      ...
      if (stream) {
        ...
        const skip =
          ...
          Array.from(this._tags).some(
            (entry) => entry[0] !== "flatbuffers" && entry[1].size > 0
          ) ||
        ...
        if (!skip) {
          switch (type) {
                ...
                if (condition) {
                  ...
                  execution.on("resolve", (_, name) => {
                    if (!torch || !name.startsWith("__torch__.")) {
                      this.exception(
                        new view.Error("Unknown type name '" + name + "'.")
                      );
                    }
                  });
            ...
            default: {
              throw new view.Error(
                "Unsupported open format type '" + type + "'."
              );
            }
          }
        }
    ...
    if (!this._tags.has(type)) {
      ...
      if (stream) {
        ...
        const skip =
          ...
          (Array.from(this._tags).some(
            (pair) => pair[0] !== "flatbuffers" && pair[1].size > 0
          ) &&
            type !== "pb+") ||
          Array.from(this._content.values()).some((obj) => obj !== undefined) ||
    ...
    const pbtxt = () => {
      const formats = [
        ...
        },
        {
          ...
          tags: ["name", "platform", "input", "output"],
        },
        {
      ...
      if (tags.size > 0) {
        ...
        entries.push(
          ...Array.from(tags).filter(
            (pair) => pair[0].toString().indexOf(".") === -1
          )
        );
        entries.push(
          ...Array.from(tags).filter(
        ...
        const content = entries
          .map((pair) =>
            pair[1] === true ? pair[0] : pair[0] + ":" + JSON.stringify(pair[1])
          )
          .join(",");
        throw new view.Error(
      ...
      const filter = async (queue) => {
        ...
        const nextEntry = async () => {
          if (queue.length > 0) {
            ...
            const context = new view.ModelContext(
              new view.EntryContext(
                this._host,
                entries,
                folder,
                entry.name,
          ...
          if (
            matches.length === 2 &&
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".params")
            ) &&
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith("-symbol.json")
            )
          ) {
          ...
          if (
            matches.length > 0 &&
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".bin")
            ) &&
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".json")
            )
          ) {
            matches = matches.filter((context) =>
              context.identifier.toLowerCase().endsWith(".json")
          ...
          if (
            ...
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".param")
            )
          ) {
            matches = matches.filter((context) =>
              context.identifier.toLowerCase().endsWith(".param")
          ...
          if (
            ...
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".param.bin")
            )
          ) {
            matches = matches.filter((context) =>
              context.identifier.toLowerCase().endsWith(".param.bin")
          ...
          if (
            matches.length > 0 &&
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".nnef")
            ) &&
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".dat")
            )
          ) {
            matches = matches.filter((context) =>
              context.identifier.toLowerCase().endsWith(".nnef")
          ...
          if (
            matches.length > 0 &&
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".pdmodel")
            ) &&
            (matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".pdparams")
            ) ||
              matches.some((context) =>
                context.identifier.toLowerCase().endsWith(".pdopt")
              ) ||
              matches.some((context) =>
                context.identifier.toLowerCase().endsWith(".pdiparams")
              ))
          ) {
            matches = matches.filter((context) =>
              context.identifier.toLowerCase().endsWith(".pdmodel")
          ...
          if (
            ...
            matches.some(
              (context) =>
                context.identifier.toLowerCase().split("/").pop() ===
                "__model__.nb"
            ) &&
            matches.some(
              (context) =>
                context.identifier.toLowerCase().split("/").pop() === "param.nb"
            )
          ) {
          ...
          if (
            matches.length > 1 &&
            matches.some((context) =>
              context.identifier.toLowerCase().endsWith(".data-00000-of-00001")
            )
          ) {
            matches = matches.filter(
              (context) =>
