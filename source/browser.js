...
host.BrowserHost = class {
  constructor() {
    ...
    this._environment = {
      ...
      date:
        Array.isArray(this._meta.date) &&
        this._meta.date.length > 0 &&
        this._meta.date[0]
          ? new Date(this._meta.date[0].split(" ").join("T") + "Z")
          : new Date(),
      packaged: this._meta.version && this._meta.version[0] !== "0.0.0",
      platform: /(Mac|iPhone|iPod|iPad)/i.test(this._navigator.platform)
        ? "darwin"
        : undefined,
      agent:
        this._navigator.userAgent.toLowerCase().indexOf("safari") !== -1 &&
        this._navigator.userAgent.toLowerCase().indexOf("chrome") === -1
          ? "safari"
          : "",
  ...
  async view(view) {
    ...
    const age = async () => {
      const days =
        (new Date() - new Date(this._environment.date)) / (24 * 60 * 60 * 1000);
      if (days > 180) {
    ...
    const telemetry = async () => {
      if (this._environment.packaged) {
        this._window.addEventListener("error", (event) => {
          const error =
            ...
            event.error instanceof Error
              ? event.error
              : new Error(
                  event && event.message ? event.message : JSON.stringify(event)
                );
        ...
        const ga4 = async () => {
          ...
          this._telemetry.set(
            "page_location",
            this._document.location && this._document.location.href
              ? this._document.location.href
              : null
          );
          this._telemetry.set(
            "page_title",
            this._document.title ? this._document.title : null
          );
          this._telemetry.set(
            "page_referrer",
            this._document.referrer ? this._document.referrer : null
          );
          ...
          this._setCookie(
            "_ga",
            "GA1.2." + this._telemetry.get("client_id"),
            1200
          );
          this._setCookie(
            "_ga" + measurement_id,
            "GS1.1." + this._telemetry.session,
            1200
          );
        };
        const ua = async () => {
    ...
    const capabilities = async () => {
      ...
      const optional = ["fetch", "DataView.prototype.getBigInt64", "Worker"];
      ...
      this.event("browser_open", {
        browser_capabilities: capabilities
          .map((capability) => capability.split(".").pop())
          .join(","),
      });
  ...
  async start() {
    ...
    if (url) {
      ...
      const location = url
        .replace(
          /^https:\/\/github\.com\/([\w-]*\/[\w-]*)\/blob\/([\w/\-_.]*)(\?raw=true)?$/,
          "https://raw.githubusercontent.com/$1/$2"
        )
        .replace(
          /^https:\/\/github\.com\/([\w-]*\/[\w-]*)\/raw\/([\w/\-_.]*)$/,
          "https://raw.githubusercontent.com/$1/$2"
        )
        .replace(
          /^https:\/\/huggingface.co\/(.*)\/blob\/(.*)$/,
          "https://huggingface.co/$1/resolve/$2"
        );
    ...
    if (openFileButton && openFileDialog) {
      ...
      const mobileSafari =
        this.environment("platform") === "darwin" &&
        navigator.maxTouchPoints &&
        navigator.maxTouchPoints > 1;
      if (!mobileSafari) {
        const base = require("./base");
        const extensions = new base.Metadata().extensions.map(
          (extension) => "." + extension
        );
      ...
      openFileDialog.addEventListener("change", (e) => {
        if (e.target && e.target.files && e.target.files.length > 0) {
          ...
          const file = files.find((file) =>
            this._view.accept(file.name, file.size)
          );
          if (file) {
  ...
  async _openGist(gist) {
    ...
    try {
      ...
      const key = Object.keys(json.files).find((key) =>
        this._view.accept(json.files[key].filename)
      );
      if (!key) {
        this.error(
          "Error while loading Gist.",
          "Gist does not contain a model file."
        );
        return;
      }
      const base = require("./base");
      ...
      const stream = new base.BinaryStream(buffer);
      const context = new host.BrowserHost.Context(
        this,
        "",
        identifier,
        stream
  ...
  _setCookie(name, value, days) {
    ...
    const location = this.window.location;
    const domain =
      location && location.hostname && location.hostname.indexOf(".") !== -1
        ? ";domain=." + location.hostname.split(".").slice(-2).join(".")
        : "";
    const date = new Date();
    date.setTime(date.getTime() + days * 24 * 60 * 60 * 1000);
    this.document.cookie =
      name + "=" + value + domain + ";path=/;expires=" + date.toUTCString();
  }
  ...
  get(context, name) {
    try {
      if (
        ...
        typeof this.window.localStorage !== "undefined"
      ) {
  ...
  _message(message, action, img) {
    return new Promise((resolve) => {
      ...
      if (img) {
        ...
        this._element("message-image").style.display = "none";
      }
          ...
          if (position === 0 && buffer.length === blob.size) {
            ...
            if (position < blob.size) {
              const slice = blob.slice(
                position,
                Math.min(position + size, blob.size)
              );
              reader.readAsArrayBuffer(slice);
            } else {
              const stream = new host.BrowserHost.FileStream(
                chunks,
                size,
                0,
                position
...
host.BrowserHost.FileStream = class {
  ...
  stream(length) {
    const file = new host.BrowserHost.FileStream(
      this._chunks,
      this._size,
  ...
  _fill(length) {
    if (this._position + length > this._length) {
      throw new Error(
        "Expected " +
          (this._position + length - this._length) +
          " more bytes. The file might be corrupted. Unexpected end of file."
      );
    }
    if (
      ...
      this._position + length > this._offset + this._buffer.length
    ) {
      this._offset = this._position;
      this._buffer = new Uint8Array(
        Math.min(0x10000000, this._size, this._length - this._offset)
      );
  ...
  _read(buffer, offset) {
    ...
    if (length !== buffer.length) {
      buffer.set(
        this._chunks[index + 1].subarray(0, buffer.length - length),
        length
      );
    }
  }
        ...
        const context = {
          ...
          x:
            typeof options.left === "undefined"
              ...
              : ~~options.left,
          y:
            typeof options.top === "undefined" ? this.scrollTop : ~~options.top,
